...
Neurodiveragents is eighteen agents built on that principle — one per domain, each grounded in a cognitive style that makes it structurally incapable of the failure modes a generic agent produces by default. For Claude Code, OpenCode, Cursor, and GitHub Copilot.

Eighteen specialists. Full routing table: [`humans/ndv-agents.md`](../humans/ndv-agents.md)
...
Full neurotype doctrine: [`humans/ndv-agents.md`](../humans/ndv-agents.md)
...
- **Specialists over generalists.** Eighteen focused agents outperform one do-everything agent because depth beats breadth in technical domains.
